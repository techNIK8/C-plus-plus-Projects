...
void RamerDouglasPeucker(const vector<Point> &pointList, double epsilon, vector<Point> &out)
{
	...
	double	dmax = 0.0;
	int	index = 0;
	int	end = pointList.size() - 1; //index tou teleutaiou simiou sto ena Polyline(vector<point>)
	...
	if (dmax > epsilon) { //diaspasi polyline, index=to simio me megaluteri apostasi apo tin eu8eia
		// Recursive call
		vector<Point> recResults1; //ypo-polyline 1
		vector<Point> recResults2; //ypo-poluline 2
		vector<Point> firstLine(pointList.begin(), pointList.begin() + index + 1); //arxi,telos ypo-polyline 1
		vector<Point> lastLine(pointList.begin() + index, pointList.end()); //arxi-telos ypo-polyline 2
...
int main(int argc, char *argv[])
{
	...
	int chunkSize=AllPolylines.size();
	int reps=0;

	#pragma omp parallel private(numOfThreads,chunkSize,reps)
	{
		#pragma omp for schedule(dynamic)
		for (int i = 0; i <= numOfThreads; ++i)
		{

			for (vector< vector<Point> >::const_iterator it1 = AllPolylines.begin()+omp_get_thread_num()*chunkSize; reps<=chunkSize; it1++) {
			reps++;
			polyline = (*it1);
